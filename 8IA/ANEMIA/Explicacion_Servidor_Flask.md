...
# ▶ Red Neuronal Anemia (carga condicional)
if os.path.exists("anemia_modelo_rn.h5") and os.path.exists("anemia_scaler.pkl"):
    anemia_nn_model = load_model("anemia_modelo_rn.h5")
    anemia_scaler   = joblib.load("anemia_scaler.pkl")
else:
    anemia_nn_model = None
    anemia_scaler   = None

# ▶ Árbol de Decisión Anemia (carga condicional)
if os.path.exists("anemia_modelo_dt.pkl"):
    anemia_dt_model = joblib.load("anemia_modelo_dt.pkl")
else:
...
| `/anemia` | GET | `anemia_form()` | Muestra el formulario de Anemia vacío |
| `/anemia/predecir` | POST | `anemia_predecir()` | Recibe datos Anemia y retorna predicción |
...
## Bloque 6: Ruta de Predicción — Anemia

La estructura es idéntica a la del Titanic, pero con las columnas y modelos de anemia:
