...
Este documento explica paso a paso cómo funcionan los scripts de entrenamiento de la **Red Neuronal** (`Anemia_RN_Local.py` y `Titanic_RN_Local.py`). Está escrito para que cualquier persona, sin importar su experiencia, pueda entender qué hace cada línea importante.
...
```python
# Modelo de Anemia (con Dropout y regularización L2)
...
- **`sigmoid`**: Toma cualquier número y lo "aplana" entre 0 y 1. Perfecta para la capa final cuando queremos interpretar el resultado como una **probabilidad** (ej: 0.87 = 87% de probabilidad de anemia).

**¿Qué es el `Dropout`?**
Durante cada iteración de entrenamiento, el Dropout "apaga" aleatoriamente un porcentaje de neuronas (30% en la primera capa, 20% en la segunda). Esto obliga a la red a no depender de ningún camino único de cálculo y a aprender patrones más generales. Sin Dropout, el dataset de anemia (que tiene clases muy bien definidas) lleva al modelo a memorizar los datos y devolver siempre 0% o 100% exactos.
...
```python
model.save("anemia_modelo_rn.h5")
```
...
Este es exactamente el mismo proceso que ocurre cuando un usuario completa el formulario web en la página de Anemia.
...
```python
# En app.py, rama de predicción de anemia con Red Neuronal:
...
**Explicación práctica:**
El dataset de anemia tiene clases muy bien definidas clínicamente, lo que hace que incluso con Dropout y L2 el modelo devuelva probabilidades muy cercanas a 0% o 100% para la mayoría de los casos. El **Temperature Scaling** es una técnica de calibración post-entrenamiento que suaviza estas predicciones extremas *sin reentrenar el modelo*.
