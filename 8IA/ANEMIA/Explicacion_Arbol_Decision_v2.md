...
> [!IMPORTANT]
> **Nota sobre Anemia_DT_Local.py:** El modelo de Anemia usa `RandomForestClassifier` (un bosque de 100 árboles) en lugar de un solo `DecisionTreeClassifier`. Esto se debe a que el dataset de anemia tiene clases perfectamente separables, lo que hacía que un árbol individual devolviera siempre 0% o 100% de probabilidad (hojas puras). El Random Forest promedia las probabilidades de 100 árboles distintos, produciendo valores más naturales e intermedios. El modelo de Titanic sí usa un único árbol de decisión.
...
| `RandomForestClassifier` | El bosque de árboles de decisión (ensemble) usado en Anemia |
...
### Versión Anemia — Random Forest (ensemble de 100 árboles)
...
El dataset de anemia tiene clases clínicamente bien separadas (la hemoglobina define casi perfectamente si hay anemia o no). Un solo árbol memoriza esto y devuelve 0% o 100% exactos (hojas puras). Al promediar 100 árboles con distintos datos de entrenamiento, obtenemos probabilidades intermedias más realistas como 67% o 28%.

```
Árbol 1: 100% anemia ┐
Árbol 2:  95% anemia  │
Árbol 3:  85% anemia  ├──► Promedio: 74% anemia ✓
...                   │
Árbol 100: 40% anemia ┘
```
